...
export default function CommentItem({ comment, postId, postAuthorId }) {
    ...
    const [isConfirmCommentDeletion, setIsConfirmCommentDeletion] = useState(false);
    ...
    const handleDeleteComment = () => document.getElementById('delete_comment_confirmation_modal').showModal();
    ...
    useEffect(() => {
        if (isConfirmCommentDeletion) {
            deleteComment(comment._id, {
                onSuccess: () => {
                    dialogFormRef.current?.submit();
                },
                onError: (err) => {
                    toast.error(err.response?.data?.error || 'Failed to delete the comment!');
                },
            });
        }
    }, [isConfirmCommentDeletion, comment._id])
    ...
    return (
        <>
            <DeleteCommentConfirmationModal
                comment={comment}
                setIsConfirmCommentDeletion={setIsConfirmCommentDeletion}
                dialogFormRef={dialogFormRef}
                isEdited={isEdited}
                formattedTimeAgo={formattedTimeAgo}
                isDeleting={isDeletePending}
                isMyPost={isMyPost}
                isMyCommentAndMyPost={isMyCommentAndMyPost}
                isMyCommentAndNotMyPost={isMyCommentAndNotMyPost}
            />
            ...
            <div className='flex flex-col gap-y-0.5'>
                ...
                <div className='flex justify-between items-start gap-x-4'>
                    <div className='flex gap-x-2 flex-1'>
                        ...
                        <div>
                            <Link to={`/profile/${comment.user.username}`} className='block'>
                                ...
                                <p className='text-[12px] line-clamp-1 text-black/60'>{comment.user.headline}</p>
                            </Link>
                        </div>
                    </div>
                    ...
                    {/* Comment Actions */}
                    {!isCommentEditing && (
                        <div className='flex items-center gap-x-1 text-black/50'>
                    ...
                    {isCommentEditing ? (
                        <>
                            <div
                                ...
                                className={`text-xs relative break-words border border-gray-500 rounded-3xl p-2.5 focus:outline-blue-500 ${editedComment.trim().length === 0 ? "before:content-['Add_a_comment'] before:absolute before:left-3 before:text-gray-500" : ""
                                    }`}
                                role='textbox'
                                dir='auto'
                                onInput={handleInput}
